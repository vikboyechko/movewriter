...
def german_layout():
    ...
    punct = [
        ...
        # Brackets → ü/Ü and +/*
        (26, 0x00FC, 0x00DC, 0x00FC, 0x00DC),  # ü / Ü
        (27, 0x2B, 0x2B, 0x2A, 0x2A),          # + / *
        # Semicolon → ö/Ö
        (39, 0x00F6, 0x00D6, 0x00F6, 0x00D6),  # ö / Ö
        # Apostrophe → ä/Ä
        (40, 0x00E4, 0x00C4, 0x00E4, 0x00C4),  # ä / Ä
        # Grave → ^/°
...
def french_layout():
    ...
    punct = [
        ...
        (39, 0x6D, 0x4D, 0x6D, 0x4D),
        # Apostrophe → ù/%
        (40, 0x00F9, 0x25, 0x00F9, 0x25),
        # Grave → ²/³
...
def portuguese_layout():
    ...
    punct = [
        ...
        (39, 0x00E7, 0x00C7, 0x00E7, 0x00C7),  # ç / Ç
...
def swedish_layout():
    ...
    punct = [
        ...
        (26, 0x00E5, 0x00C5, 0x00E5, 0x00C5),  # å / Å
...
def norwegian_layout():
    ...
    punct = [
        ...
        (26, 0x00E5, 0x00C5, 0x00E5, 0x00C5),  # å / Å
        (27, 0x00A8, 0x00A8, 0x5E, 0x5E),   # ¨ / ^  (dead)
        (39, 0x00F8, 0x00D8, 0x00F8, 0x00D8),  # ø / Ø
        (40, 0x00E6, 0x00C6, 0x00E6, 0x00C6),  # æ / Æ
...
def danish_layout():
    ...
    punct = [
        ...
        (26, 0x00E5, 0x00C5, 0x00E5, 0x00C5),  # å / Å
        (27, 0x00A8, 0x00A8, 0x5E, 0x5E),   # ¨ / ^  (dead)
        (39, 0x00E6, 0x00C6, 0x00E6, 0x00C6),  # æ / Æ
        (40, 0x00F8, 0x00D8, 0x00F8, 0x00D8),  # ø / Ø
...
def swiss_french_layout():
    ...
    punct = [
        ...
        (12, 0x27, 0x27, 0x3F, 0x3F),       # ' / ?
        (13, 0x5E, 0x5E, 0x60, 0x60),       # ^ / `  (dead)
        (26, 0x00E8, 0x00C8, 0x00E8, 0x00C8),  # è / È
        (27, 0x00A8, 0x00A8, 0x21, 0x21),   # ¨ / !
        (39, 0x00E9, 0x00C9, 0x00E9, 0x00C9),  # é / É
        (40, 0x00E0, 0x00C0, 0x00E0, 0x00C0),  # à / À
        (41, 0x00A7, 0x00A7, 0x00B0, 0x00B0),  # § / °
        (43, 0x24, 0x24, 0x00A3, 0x00A3),   # $ / £
        (51, 0x2C, 0x2C, 0x3B, 0x3B),       # , / ;
        (52, 0x2E, 0x2E, 0x3A, 0x3A),       # . / :
...
def belgian_layout():
    ...
    punct = [
        ...
        (39, 0x6D, 0x4D, 0x6D, 0x4D),        # m / M  (letter on punct key)
        (40, 0x00F9, 0x25, 0x00F9, 0x25),    # ù / %
        (41, 0x00B2, 0x00B2, 0x00B3, 0x00B3),  # ² / ³
...
def russian_layout():
    ...
    punct = [
        (26, 0x0445, 0x0425, 0x0445, 0x0425),  # [ → х/Х
        (27, 0x044A, 0x042A, 0x044A, 0x042A),  # ] → ъ/Ъ
        (39, 0x0436, 0x0416, 0x0436, 0x0416),  # ; → ж/Ж
        (40, 0x044D, 0x042D, 0x044D, 0x042D),  # ' → э/Э
        (41, 0x0451, 0x0401, 0x0451, 0x0401),  # ` → ё/Ё
        (43, 0x5C, 0x5C, 0x2F, 0x2F),          # \ / /
        (51, 0x0431, 0x0411, 0x0431, 0x0411),  # , → б/Б
        (52, 0x044E, 0x042E, 0x044E, 0x042E),  # . → ю/Ю
...
def ukrainian_layout():
    ...
    punct = [
        (26, 0x0445, 0x0425, 0x0445, 0x0425),  # [ → х/Х
        (27, 0x0457, 0x0407, 0x0457, 0x0407),  # ] → ї/Ї  (Ukrainian Yi)
        (39, 0x0436, 0x0416, 0x0436, 0x0416),  # ; → ж/Ж
        (40, 0x0454, 0x0404, 0x0454, 0x0404),  # ' → є/Є  (Ukrainian Ie)
        (41, 0x0027, 0x0027, 0x007E, 0x007E),  # ` → ' / ~  (apostrophe)
        (43, 0x0491, 0x0490, 0x0491, 0x0490),  # \ → ґ/Ґ  (Ukrainian Ghe with upturn)
        (51, 0x0431, 0x0411, 0x0431, 0x0411),  # , → б/Б
        (52, 0x044E, 0x042E, 0x044E, 0x042E),  # . → ю/Ю
...
def hungarian_layout():
    ...
    punct = [
        ...
        (9,  0x38, 0x38, 0x28, 0x28),       # 8 / (
        (10, 0x39, 0x39, 0x29, 0x29),       # 9 / )
        (11, 0x00F6, 0x00D6, 0x00F6, 0x00D6),  # ö / Ö
        (12, 0x00FC, 0x00DC, 0x00FC, 0x00DC),  # ü / Ü
        (13, 0x00F3, 0x00D3, 0x00F3, 0x00D3),  # ó / Ó
        (26, 0x0151, 0x0150, 0x0151, 0x0150),  # ő / Ő
        (27, 0x00FA, 0x00DA, 0x00FA, 0x00DA),  # ú / Ú
        (39, 0x00E9, 0x00C9, 0x00E9, 0x00C9),  # é / É
        (40, 0x00E1, 0x00C1, 0x00E1, 0x00C1),  # á / Á
        (41, 0x30, 0x30, 0x00A7, 0x00A7),   # 0 / §
        (43, 0x0171, 0x0170, 0x0171, 0x0170),  # ű / Ű
        (51, 0x2C, 0x2C, 0x3F, 0x3F),       # , / ?
        (52, 0x2E, 0x2E, 0x3A, 0x3A),       # . / :
        (53, 0x2D, 0x2D, 0x5F, 0x5F),       # - / _
    ]
...
def turkish_layout():
    ...
    punct = [
        ...
        (13, 0x2D, 0x2D, 0x5F, 0x5F),       # - / _
        (26, 0x011F, 0x011E, 0x011F, 0x011E),  # ğ / Ğ
        (27, 0x00FC, 0x00DC, 0x00FC, 0x00DC),  # ü / Ü
        (39, 0x015F, 0x015E, 0x015F, 0x015E),  # ş / Ş
        (40, 0x69, 0x0130, 0x69, 0x0130),    # i / İ  (dotted small i / capital İ)
        (41, 0x22, 0x22, 0x00E9, 0x00E9),    # " / é
        (43, 0x2C, 0x2C, 0x3B, 0x3B),        # , / ;
        (51, 0x00F6, 0x00D6, 0x00F6, 0x00D6),  # ö / Ö
        (52, 0x00E7, 0x00C7, 0x00E7, 0x00C7),  # ç / Ç
